...
class BackendSession {
  ...
  async startRun(input: {
    ...
    if (!accountId || !backend) {
      return { success: false, error: "Not connected to Mains on your computer yet" };
    }
    if (!selectedSpaceId) {
    ...
    const space = db
      ...
      .get();
    if (!space) {
      return { success: false, error: "That space no longer exists in Mains on your computer" };
    }
    if (
  ...
  readRunTextFile(runId: string, filePath: string): Promise<RunTextFile> {
    if (!this.isConnected() || !this.transport) {
      return Promise.reject(new Error("Connect to Mains on your computer to load this document"));
    }
    ...
    return new Promise((resolve, reject) => {
      const check = (): boolean => {
        ...
        if (
          ...
          reject(new Error(`Not connected to Mains on your computer (${connection.kind})`));
          return true;
        }
        return false;
      };
