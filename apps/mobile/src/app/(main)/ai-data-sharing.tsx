...
export default function AiDataSharingScreen() {
  ...
  const revoke = (providerId: string, serviceName: string) => {
    Alert.alert(
      ...
      `Mains will ask again before this computer sends data to ${serviceName}.`,
      [
  ...
  const resetAll = () => {
    Alert.alert(
      ...
      "Mains will ask again before this computer sends data to any AI provider.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Reset",
  ...
  return (
    ...
    >
      <ThemedText variant="subhead" selectable style={{ color: colors.secondaryLabel }}>
        Before Mains on your computer sends a request to a third-party AI service,
        Mains asks for permission for that computer and provider. Switching providers asks
        ...
        <Card>
          <View style={{ padding: spacing.md, gap: spacing.xs }}>
            ...
            <ThemedText variant="footnote" style={{ color: colors.secondaryLabel }}>
              Provider permissions appear here after you pair this phone with Mains on a computer.
            </ThemedText>
          </View>
        </Card>
      )}
