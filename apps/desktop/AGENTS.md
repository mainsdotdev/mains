...
**Menu Bar (tray)** (`src/main/tray/`)
- Not a domain module: app shell like `windows/`, shown/hidden by `showTray` / `hideTray` (setting `showMenuBarIcon`). Running-run count as the icon title, a badged icon while a request waits (`drawBadge` on the template bitmap)
- The menu is a pure function (`tray-menu.ts` → `buildTrayMenu(snapshot, actions, now)`) built fresh on every open: Needs you (pending requests answered with the same `describeApprovalNotification` / `responseFromNotification` as the notification), Running (Open / Stop), Recently finished, next pulse, remote access, New Chat, Lens capture, Prevent Sleep, update
